local Meirin, super = Class(Shop, "shop_meirin")
...
function Meirin:init()
    ...
    self:registerItem("tanghulu", {stock = 16})
    self:registerItem("egg_pancake_roll", {name = "Egg Pancake Roll", stock = 8})
    self:registerItem("halloween_umbrella_sleeve", {name = "Halloween Sleeve", stock = 4})
    self:registerItem("bat_pendant", {stock = 6})
...
function Meirin:startTalk(talk)
    ...
    elseif talk == "Why not hate her?" then
        self:startDialogue({
            ...
            "[emote:left]* A while ago, something unusual suddenly occurred inside the mansion...\nA dark fountain appeared.",
            "* Lady Patchouli was drawn to it and decided to keep it for study.",
            "* But as time went on, everyone in the mansion started acting strangely.\n[wait:5]* Even I, who usually live outside the mansion, was somewhat affected.",
