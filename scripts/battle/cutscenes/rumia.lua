return {
	heal = function(cutscene, encounter, enemy)
		...
		cutscene:battlerText('rin',"[speed:0.5]什...什么?!   ",{wait=false,auto=true,right=true})
		...
		Game.battle.timer:every(0.1,function()
			...
			Game.battle.timer:tween(1,darkeffect,{x=enemy.x-150,y=enemy.y-120,alpha=1},nil,function()
				Game.battle.timer:tween(0.5,darkeffect,{alpha=0},nil,function()darkeffect:remove()end)
			end)
			if darkeffectcount==8 then
				return false
			end
		end)
		Game.battle.timer:after(2.5,function()enemy:heal(enemy.max_health)Game.battle.timer:after(0.5,function()enemy:setAnimation('battle/idle',1/10,true)enemy:shake(6,0,3)end)end)
		cutscene:text("* 露米娅过于紧张，吸收了大量黑暗。                                          ",nil,nil,{skip=false,auto=true})
		cutscene:text("* 露米娅的攻击力上升了。")
		cutscene:battlerText('rin',"正邪，别打了，\n你让她变得\n更强了!",{right=true})
		cutscene:battlerText('seija',"让她认为我怕了?",{right=true})
		cutscene:battlerText('seija',"呸，我才不干!",{right=true})
